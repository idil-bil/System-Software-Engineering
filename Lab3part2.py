...
if __name__ == "__main__":
    ...
    for num in range(SIZE):                                                                              #creates the processes in a loop to check all 27 cases
        ColumnCheck = Process(target = checkColumn, args = (testcase, num,))
        RowCheck = Process(target = checkRow, args = (testcase, num,))
        SubgridCheck = Process(target = checkSubgrid, args = (testcase, num,))

        validation = [[Process(target=checkColumn, args=(testcase, column,)) for column in range(SIZE)],  #saves them to call them outside the loop for faster multiprocessing
                      [Process(target=checkRow, args=(testcase, row,)) for row in range(SIZE)],
                      [Process(target=checkSubgrid, args=(testcase, subgrid,)) for subgrid in range(SIZE)]
                     ]

        ColumnCheck.start()                                                                                #starts each process
        RowCheck.start()
        SubgridCheck.start()

    for processnumber in range(3):                                                                          #joins the processes in a loop
        for index in range(SIZE):
            validation[processnumber][index].join()

    #DOING THIS INSTEAD OF THE FOR LOOP ABOVE SLOWS IT DOWN
    # ColumnCheck.join()
    # RowCheck.join()
    # SubgridCheck.join()

    #THIS IS AN ALTERNATIVE VERSION TO DO THE MULTIPROCESSING
    # validation = [[Process(target=checkColumn, args=(testcase, column,)) for column in range(SIZE)],
    #               [Process(target=checkRow, args=(testcase, row,)) for row in range(SIZE)],
    #               [Process(target=checkSubgrid, args=(testcase, subgrid,)) for subgrid in range(SIZE)]
    #              ]

    # for processnumber in range(3):
    #     for index in range(SIZE):
    #         validation[processnumber][index].start()
